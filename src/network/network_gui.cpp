				...
				NWidget(NWID_VERTICAL), SetPIP(0, 7, 0),
					...
					NWidget(WWT_PANEL, COLOUR_LIGHT_BLUE, WID_NG_DETAILS),
						NWidget(NWID_VERTICAL, NC_EQUALSIZE), SetPIP(5, 5, 5),
							NWidget(WWT_EMPTY, INVALID_COLOUR, WID_NG_DETAILS_SPACER), SetMinimalSize(140, 0), SetMinimalTextLines(9, 24 + WD_PAR_VSEP_NORMAL), SetResize(0, 1), SetFill(1, 1), // Make sure it's at least this wide
		...
		switch (widget) {
			case WID_NSS_CONNTYPE_BTN:
				*size = maxdim(GetStringBoundingBox(_connection_types_dropdown[0]), GetStringBoundingBox(_connection_types_dropdown[1]));
				size->width = GetMinButtonSize(size->width + padding.width);
				size->height = GetMinButtonSize(size->height + padding.height);
				break;
		}
	}
...
struct NetworkClientListPopupWindow : Window {
	...
	{
		this->desired_location.x = x - GetMinButtonSize(FONT_HEIGHT_NORMAL);
		this->desired_location.y = y + GetMinButtonSize(FONT_HEIGHT_NORMAL) / 2;
	...
	void UpdateWidgetSize(int widget, Dimension *size, const Dimension &padding, Dimension *fill, Dimension *resize) override
	{
		...
		d.height = GetMinButtonSize(FONT_HEIGHT_NORMAL);
	...
	void DrawWidget(const Rect &r, int widget) const override
	{
		...
		for (const ClientListAction &action : this->actions) {
			...
			if (sel-- == 0) { // Selected item, highlight it
				GfxFillRect(r.left + 1, y, r.right - 1, y + GetMinButtonSize(FONT_HEIGHT_NORMAL) - 1, PC_BLACK);
				colour = TC_WHITE;
			} else {
				colour = TC_BLACK;
			}

			DrawString(r.left + WD_FRAMERECT_LEFT, r.right - WD_FRAMERECT_RIGHT, Center(y, GetMinButtonSize(FONT_HEIGHT_NORMAL)), action.name, colour);
	...
	void OnClick(Point pt, int widget, int click_count) override
	{
		int index = (pt.y - WD_FRAMERECT_TOP) / GetMinButtonSize(FONT_HEIGHT_NORMAL);
